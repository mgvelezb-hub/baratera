...
function ProductoCardPOS({
  ...
  return (
    <div className={`rounded-xl border overflow-hidden transition-all ${
      ...
      {tieneColores && (
        <div className="border-t border-slate-100 px-2 py-1.5 flex flex-wrap gap-1.5">
          {colores.map(c => {
            ...
            return (
              <button
                ...
                className={`w-5 h-5 rounded-full border-2 transition-all ${
                  pzsEnCarrito > 0
                    ? 'border-violet-500 scale-110 shadow-sm'
                    : agotado
                      ? 'border-white opacity-30 cursor-not-allowed'
                      : 'border-white shadow-sm hover:scale-110 active:scale-95'
                }`}
